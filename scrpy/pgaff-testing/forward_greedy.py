...
with open('secr/Sample Average Approximation/SA2/SA2-1/runtime.txt', 'w') as f:
...
np.save('./secr/Sample Average Approximation/SA2/SA2-1/all_selected_traps.npy', selected_traps)
...
with open('./secr/Sample Average Approximation/SA2/SA2-1/rse_hist.txt', 'w') as f:
    for rse in RSE_hist:
        f.write(f"{rse}\n")
# Save the trap_x configuration
with open('./secr/Sample Average Approximation/SA2/SA2-1/considered_trap_locs.pkl', 'wb') as f:
...
trap_coords_list_df = pd.DataFrame(trap_coords_list, columns=['x', 'y'])
trap_coords_list_df.to_csv(f'./secr/Sample Average Approximation/SA2/SA2-1/considered_trap_locs.csv', index=False)

# Generate Files for SECR Analysis
base_dir = './secr/Sample Average Approximation/SA2/SA2-1'
# trap_coords_list = pd.read_csv(f'./secr/Sample Average Approximation/SA2/SA2-1/considered_trap_locs.csv')
...
for n_cams in [10, 20, 30, 40, 50, 60, 70, 80]:
    ...
    excluded_txt_path = f'{base_dir}/SA2/SA2-1-excluded_traps-{n_cams}.txt'
