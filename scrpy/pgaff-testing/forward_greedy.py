...
def forward_greedy_max_en(scenarios, trap_locs, ac_locs, K, distances, draw, draw_to_trueN, budget=100):
    ...
    with open('secr/Sample Average Approximation/SA1/SA1-17/n_tracker.txt', 'w') as f:
...
params_ids =  [96, 84, 19, 88, 72, 7, 16, 43, 50, 46]
...
with open('secr/Sample Average Approximation/SA1/SA1-17/runtime.txt', 'w') as f:
...
# Save the results
np.save('./secr/Sample Average Approximation/SA1/SA1-17/all_selected_traps.npy', selected_traps)
# Save the expected number of detections history as a txt file
with open('./secr/Sample Average Approximation/SA1/SA1-17/en_hist.txt', 'w') as f:
    for en in en_hist:
        f.write(f"{en}\n")
# Save the trap_x configuration
with open('./secr/Sample Average Approximation/SA1/SA1-17/considered_trap_locs.pkl', 'wb') as f:
...
# Pathsx
base_dir = './secr/Sample Average Approximation/SA1/SA1-17'
...
for n_cams in [10, 20, 30, 40, 50, 60, 70, 80]:
    ...
    excluded_txt_path = f'{base_dir}/SA1-17-excluded_traps-{n_cams}.txt'
