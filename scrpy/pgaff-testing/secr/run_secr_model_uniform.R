...
write.csv(traps12 %>% dplyr::select(Trap_index, x, y), 
          "U_60A_1km_SAA2_SelectedTrapCoordinates.csv", 
          row.names = FALSE)
...
bbox<-st_bbox(SA) #This gets the rectangular bounding box of the study area
...
true_N <- read.csv("True_N_per_draw_marten.csv")
...
for (draw in draw_ids) {
    ...
    tryCatch({
        ...
        saveRDS(fit_model, file = paste("model_U_60A_1km_SAA2.RDS"))
...
file_name <- paste0("U_60A_1km_SAA2_", start_draw, "-", end_draw, ".csv")
