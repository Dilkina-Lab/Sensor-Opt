...
# exclude_traps <- read.table("SensorOpt/secr/Prior Deployment/P2/P2-excluded_traps.txt")  # Assumes one ID per line
...
############### IMPORTANT:
# Use secr v5.2.1 or later recommended
###############
...
exclude_traps <- read.table("SensorOpt/secr/Forward Greedy/F17/FG17-excluded_traps.txt", col.names = "Trap_index")
...
file_name <- paste0("FG17_", start_draw, "-", end_draw, ".csv")
write.csv(results, file = file_name, row.names = FALSE)
