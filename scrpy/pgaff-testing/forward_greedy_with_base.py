...
#     with open('./secr/Forward Greedy/FG52/n_tracker.txt', 'w') as f:
...
params = params.sample(n=50).reset_index(drop=True)                          # Randomly select 50 draws
...
exclude_indices.update(range(29, 38))
exclude_indices.update(range(712, 716))
exclude_indices.update(range(746,760))
exclude_indices.update(range(1182,1188))
exclude_indices.update(range(777,880))
exclude_indices.update(range(831,840))
exclude_indices.update(range(1,9))
...
trap_df.to_csv('./secr/Forward Greedy/FG52/considered_trap_locs.csv', index=False)
print(f"Total traps including deployed + candidates: {len(combined_traps)}")

# Indices of deployed traps = first 70
...
with open('./secr/Forward Greedy/FG52/runtime.txt', 'w') as f:
...
# Save results
np.save('./secr/Forward Greedy/FG52/all_selected_traps.npy', selected_traps)
with open('./secr/Forward Greedy/FG52/en_hist.txt', 'w') as f:
    for e in en_hist:
        f.write(f"{e}\n")
with open('./secr/Forward Greedy/FG52/considered_trap_locs.pkl', 'wb') as f:
...
final_df.to_csv('./secr/Forward Greedy/FG52/selected_traps.csv', index=False)
...
with open('./secr/Forward Greedy/FG52/FG52-excluded_traps.txt', 'w') as f:
