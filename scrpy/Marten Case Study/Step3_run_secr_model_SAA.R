...
traps_500m <- read.csv("scrpy/Marten Case Study/Marten Data/1000m_trap_grid_marten.csv")
...
exclude_traps <- read.table("scrpy/Marten Case Study/Marten Data/Optimized Layouts/SA2-1/SA2-1-excluded_traps-70.txt", col.names = "Trap_index")         # Replace based on layout you want to evaluate.
...
########################## Make a mask #########################################
param_vals <- read.csv("scrpy/Marten Case Study/Marten Data/param_values_for_each_draw300_marten.csv")
...
############################# load up a ch #####################################
true_N <- read.csv("scrpy/Marten Case Study/Marten Data/True_N_per_draw.csv")

# Evaluate either on the pre-determined validation set or the test set (see below for draw IDs for each)
...
for (draw in draw_ids) {
    gc()
    print(draw)
    ch <- read.csv(paste0("scrpy/Marten Case Study/Marten Data/ch/ch_draw_", draw, ".csv"))
...
file_name <- paste0("Marten_Test_Results_SA1_70Traps.csv")
